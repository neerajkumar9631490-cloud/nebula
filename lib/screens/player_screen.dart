...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  String _phaseText() => switch (_phase) { TorrentPhase.engine => 'Starting engine…', TorrentPhase.metadata => _isTorrent ? 'Fetching torrent info…' : 'Loading…', TorrentPhase.peers => 'Connecting to peers…', TorrentPhase.ready => 'Starting…', TorrentPhase.error => 'Engine could not start' };
  ...
  Future<void> _tryResume() async {
    ...
    if (progress != null && progress.isResumable && mounted) {
      ...
      if (_duration.inMilliseconds > 0) _player.seek(r); else { await Future.delayed(const Duration(milliseconds: 600)); if (mounted && _duration.inMilliseconds > 0) _player.seek(r); }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: Colors.black, body: Stack(children: [
      GestureDetector(onTap: _toggleControls, child: Center(child: Video(controller: _videoController, controls: (state) => const SizedBox.shrink()))),
      if (!_opened && !_failed) Center(child: Column(mainAxisSize: MainAxisSize.min, children: [const CircularProgressIndicator(strokeWidth: 2), const SizedBox(height: 14), Text(_phaseText(), style: const TextStyle(color: Colors.white70, fontSize: 13)), if (_isTorrent) ValueListenableBuilder<TorrentStats?>(valueListenable: _stats, builder: (c, s, _) => s == null ? const SizedBox.shrink() : Padding(padding: const EdgeInsets.only(top: 4), child: Text('${s.speedLabel} • ${s.activePeers} peers', style: const TextStyle(color: Colors.white38, fontSize: 11))))])),
      if (_opened && _buffering && !_failed) const Center(child: Opacity(opacity: 0.5, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white70))),
      if (_failed) Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.error_outline, size: 48, color: Colors.orangeAccent), const SizedBox(height: 8), const Text('Stream failed to load', style: TextStyle(color: Colors.white70)), if (_isTorrent) ...[const SizedBox(height: 6), Text('Last stage: ${_phaseText()}\nTip: pick a source with more peers.', textAlign: TextAlign.center, style: const TextStyle(color: Colors.white38, fontSize: 12))], const SizedBox(height: 16), FilledButton.icon(onPressed: _open, icon: const Icon(Icons.refresh), label: const Text('Retry'))]))),
      if (_controlsVisible && !_failed) Positioned(top: 0, left: 0, right: 0, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), color: Colors.black54, child: SafeArea(bottom: false, child: Row(children: [IconButton(icon: const Icon(Icons.arrow_back), onPressed: () => Navigator.pop(context)), Expanded(child: Text(widget.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.bold)))])))),
      if (_controlsVisible && !_failed) Positioned(bottom: 0, left: 0, right: 0, child: Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), color: Colors.black54, child: SafeArea(top: false, child: Column(mainAxisSize: MainAxisSize.min, children: [
        Row(children: [Text(_fmt(_position), style: const TextStyle(fontSize: 12)), Expanded(child: Slider(value: _duration.inMilliseconds > 0 ? _position.inMilliseconds.clamp(0, _duration.inMilliseconds).toDouble() : 0, max: _duration.inMilliseconds > 0 ? _duration.inMilliseconds.toDouble() : 1, onChanged: _duration.inMilliseconds > 0 ? (v) => _player.seek(Duration(milliseconds: v.toInt())) : null)), Text(_fmt(_duration), style: const TextStyle(fontSize: 12))]),
        Row(children: [IconButton(icon: const Icon(Icons.replay_10), onPressed: () => _seekBy(-10)), IconButton(icon: Icon(_playing ? Icons.pause : Icons.play_arrow, size: 36), onPressed: () => _player.playOrPause()), IconButton(icon: const Icon(Icons.forward_10), onPressed: () => _seekBy(10)), const Spacer(), if (_isTorrent) ValueListenableBuilder<TorrentStats?>(valueListenable: _stats, builder: (c, s, _) => s == null ? const SizedBox.shrink() : Padding(padding: const EdgeInsets.only(right: 10), child: Text('${s.speedLabel} • ${s.activePeers}', style: const TextStyle(fontSize: 11, color: Colors.white60)))), Icon(_volume == 0 ? Icons.volume_off : Icons.volume_down, size: 18), SizedBox(width: 90, child: Slider(value: _volume.clamp(0, 100), max: 100, onChanged: (v) => _player.setVolume(v))), PopupMenuButton<double>(onSelected: (v) => _player.setRate(v), itemBuilder: (_) => [0.5, 0.75, 1.0, 1.25, 1.5, 2.0].map((v) => PopupMenuItem(value: v, child: Text('${v}x'))).toList(), child: Padding(padding: const EdgeInsets.all(8), child: Text('${_rate}x', style: const TextStyle(fontWeight: FontWeight.bold))))]),
      ])))),
    ]));
  }
}
